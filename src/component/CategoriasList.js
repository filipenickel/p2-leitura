...
class CategoriasList extends Component {
  ...
  render() {
    ...
    return (
      ...
      <Menu>
       <Menu.Item><h3>Categorias</h3></Menu.Item>
        
          <Menu.Item><Link to="/">Todas</Link></Menu.Item>
          {categorias !== undefined && categorias.map((categoria) => (
            <Menu.Item key={categoria.name}><Link to="#" onClick={this.handleSelecionarCategoria} categoria={categoria.name}>{capitalize(categoria.name)}</Link></Menu.Item>
          ))}
